...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    {
        $this->_init('Codilar\Bannerslider\Model\Banner', 'Codilar\Bannerslider\Model\ResourceModel\Banner');
    }
    ...
    public function __construct(
        ...
        \Codilar\Bannerslider\Model\Slider $slider,
        $connection = null,
    ...
    public function joinSliderTitle()
    {
        $this->getSelect()->joinLeft(
            ['sliderTable' => $this->getTable('Codilar_bannerslider_slider')],
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        ...
        if (in_array($field, $attributes) && $storeViewId) {
            if (!in_array($field, $this->_addedTable)) {
                ...
                $this->getSelect()
                    ->joinLeft(array($field => $this->getTable('Codilar_bannerslider_value')), $sql, array());
    ...
    public function getBannerCollection($sliderId)
    {
        ...
        /** @var \Codilar\Bannerslider\Model\ResourceModel\Banner\Collection $bannerCollection */
        $bannerCollection = $this->setStoreViewId($storeViewId)
            ->addFieldToFilter('slider_id', $sliderId)
            ->addFieldToFilter('status', \Codilar\Bannerslider\Model\Status::STATUS_ENABLED)
            ->addFieldToFilter('start_time', ['lteq' => $dateTimeNow])
            ->addFieldToFilter('end_time', ['gteq' => $dateTimeNow])
            ->setOrder('order_banner', 'ASC');

        if ($this->_slider->getSortType() == \Codilar\Bannerslider\Model\Slider::SORT_TYPE_RANDOM) {
